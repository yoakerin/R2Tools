...
/**
 * 任意View边沿渐变透明  嵌套可滚动的布局（Scrollview、NestedScrollView）更佳。
 */
public class R2EdgeTransparentLayout extends FrameLayout {
    private Paint mPaint;
    private int position;
    private float drawSize;

    private int topMask = 0x01;
    private int bottomMask = topMask << 1;   //2
    private int leftMask = topMask << 2;    //4
    private int rightMask = topMask << 3;   //8

    private int mWidth;
    private int mHeight;

    public R2EdgeTransparentLayout(Context context) {
        this(context, null);
    }

    public R2EdgeTransparentLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public R2EdgeTransparentLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.R2EdgeTransparentLayout);
        position = typedArray.getInt(R.styleable.R2EdgeTransparentLayout_r2_edge_position, 0);
        drawSize = typedArray.getDimension(R.styleable.R2EdgeTransparentLayout_r2_edge_width,
