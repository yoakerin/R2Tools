package com.yoake.widgets.progress
...
class R2CountDownProgressView(context: Context, attrs: AttributeSet? = null) :
...
//            field = value
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        countTime =
            attr.getInt(R.styleable.R2CountDownProgressView_r2cd_countTime, DEFAULT_COUNT_TIME.toInt())
                .toLong()
        attr.recycle()
...
//        val sideLength = Math.min(availableWidth, availableHeight)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        when (textStyle) {
            TextStyle.SECOND ->
                displayText = if (titleCenterText!!.contains("%")) {
                    String.format(
                        titleCenterText!!,
                        (countTime * (1 - mCurrentValue) / 1000).toInt()
                    )
                } else {
                    (countTime * (1 - mCurrentValue) / 1000).toInt().toString() + "s"
                }
        ...
        if (!TextUtils.isEmpty(displayText)) {
            ...
            canvas.drawText(
                displayText ?: "",
                0 - middle / 2,
                0 - (mTextPaint.descent() + mTextPaint.ascent()) / 2,
                mTextPaint
            )
        }
