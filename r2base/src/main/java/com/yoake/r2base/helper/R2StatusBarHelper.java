...
public class R2StatusBarHelper {
    ...
    private static boolean supportTranslucent() {
        ...
        return !(R2DeviceHelper.isEssentialPhone() && Build.VERSION.SDK_INT < 26);
    }
    ...
    public static void translucent(Window window, @ColorInt int colorOn5x) {
        ...
        if (R2NotchHelper.isNotchOfficialSupport()) {
            handleDisplayCutoutMode(window);
        }

        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
            // 小米 Android 6.0 ，开发版 7.7.13 及以后版本设置黑色字体又需要 clear FLAG_TRANSLUCENT_STATUS, 因此还原为官方模式
            if (R2DeviceHelper.isFlymeLowerThan(8) || (R2DeviceHelper.isMIUI() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M)) {
...
//                window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
    ...
    private static boolean isMIUICustomStatusBarLightModeImpl() {
        if (R2DeviceHelper.isMIUIV9() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            return true;
        }
        return R2DeviceHelper.isMIUIV5() || R2DeviceHelper.isMIUIV6() ||
                R2DeviceHelper.isMIUIV7() || R2DeviceHelper.isMIUIV8();
    }
    ...
    public static boolean FlymeSetStatusBarLightMode(Window window, boolean light) {
        ...
        if (window != null) {
            ...
            if (R2DeviceHelper.isFlymeLowerThan(7)) {
                try {
            ...
            } else if (R2DeviceHelper.isFlyme()) {
                result = true;
            }
        }
        return result;
    ...
    private static void initStatusBarHeight(Context context) {
        ...
        if (sStatusBarHeight <= 0) {
            if (sVirtualDensity == -1) {
                sStatusBarHeight = R2DisplayHelper.dp2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP);
            } else {
