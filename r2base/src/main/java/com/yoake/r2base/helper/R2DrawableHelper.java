...
public class R2DrawableHelper {
    private static final String TAG = R2DrawableHelper.class.getSimpleName();
    ...
    public static Bitmap createBitmapFromView(View view, int leftCrop, int topCrop, int rightCrop, int bottomCrop) {
        Bitmap originBitmap = R2DrawableHelper.createBitmapFromView(view);
    ...
    Drawable getVectorDrawable(Context context, @DrawableRes int resVector) {
        try {
            ...
            R2Log.d(TAG, "Error in getVectorDrawable. resVector=" + resVector + ", resName=" + context.getResources().getResourceName(resVector) + e.getMessage());
            return null;
        }
    }
